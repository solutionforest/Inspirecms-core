...
class LayersPanel extends Component
{
    ...
    {
        $this->selectedBlockId = $blockId;
        $this->dispatch('select-block', blockId: $blockId)->to(VisualEditor::class);
    }

    public function hoverBlock(?string $blockId): void
    {
        $this->hoveredBlockId = $blockId;
        $this->dispatch('hover-block', blockId: $blockId)->to(VisualEditor::class);
    }

    public function deleteBlock(string $blockId): void
    {
        $this->dispatch('delete-block', blockId: $blockId)->to(VisualEditor::class);
    }

    public function duplicateBlock(string $blockId): void
    {
        $this->dispatch('duplicate-block', blockId: $blockId)->to(VisualEditor::class);
    }

    public function moveBlock(string $blockId, string $newParentId, int $position): void
    {
        $this->dispatch('move-block', blockId: $blockId, newParentId: $newParentId, position: $position)->to(VisualEditor::class);
    }

    public function render(): View
    {
        return view('inspirecms::visual-editor.livewire.layers-panel');
    }
}
