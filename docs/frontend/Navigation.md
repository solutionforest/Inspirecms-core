...
```php
<!-- resources/views/components/inspirecms/your-theme/navigation.blade.php -->
...
```php
<!-- resources/views/components/inspirecms/your-theme/mobile-navigation.blade.php -->
@props(['locale' => app()->getLocale()])
...
For advanced navigation needs, create a mega menu component:

```php
<!-- resources/views/components/inspirecms/your-theme/mega-menu.blade.php -->
@props(['locale' => app()->getLocale()])
...
<div class="mega-menu-wrapper">
    <nav class="mega-menu">
        <ul class="primary-nav">
            @foreach(inspirecms()->getNavigation('main', $locale) as $item)
                <li @class([
                        'active' => url()->current() == $item->getUrl(),
                        'has-mega-menu' => $item->hasChildren(),
                    ])
                >
                    <a href="{{ $item->getUrl() }}">{{ $item->getTitle() }}</a>

                    @if($item->hasChildren() && count($item->children) > 5)
                        <div class="mega-menu-content">
                            <div class="container">
                                <div class="row">
                                    @php
                                        $children = $item->children;
                                        $chunkedChildren = collect($children)->chunk(ceil(count($children) / 3));
                                    @endphp

                                    @foreach($chunkedChildren as $chunk)
                                        <div class="col-md-4">
...
-   [Layouts Documentation](./Layouts.md) for integrating navigation in page layouts
-   [Templates Documentation](./Templates.md) for using navigation in page templates
-   [Components Documentation](./Components.md) for building navigation components
