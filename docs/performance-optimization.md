...
path: docs/v1/performance-optimization
uri: /docs/1.x/performance-optimization
---
...
> ![NOTE]
> Clear these caches during development with the corresponding **clear** commands: 
> `php artisan route:clear`, 
> `php artisan config:clear`, and 
...
```bash {title="Start queue worker (on server)"}
php artisan queue:work --queue=high,default,low --tries=3
```

```vim {title="For use with Supervisor"}
...
```bash {title="Using k6 for load testing"}
k6 run -u 50 -d 30s tests/performance/load-homepage.js
```

Example test script:

```javascript {title="tests/performance/load-homepage.js"}
