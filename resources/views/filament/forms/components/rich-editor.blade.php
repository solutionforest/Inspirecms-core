            ...
            <div
                ...
                @if ($hasToolbarButton('mediaPicker'))
                    ...
                    x-on:media-picker-trix-appead.window="() => {
                        const element = $refs.trix;
                        if (element && $event?.detail?.statePath === @js($statePath)) {
                            $insertRichMediaPicker(element, $event.detail.data);
                        }
                    }"
                ...
                @if ($hasToolbarButton('contentPicker'))
                    x-on:content-picker-trix-click="$wire.mountFormComponentAction(@js($statePath), @js($selectContentActionName))"
                    x-on:content-picker-trix-appead.window="() => {
                        const element = $refs.trix;
                        if (element && $event?.detail?.statePath === @js($statePath)) {
                            $insertRichContentPicker(element, $event.detail.data);
                        }
                    }"
                @endif
            ...
            >
                ...
                >
                    <div class="flex gap-x-3 overflow-x-auto">
                        ...
                        @if ($hasToolbarButton(['contentPicker', 'mediaPicker']))
                            <x-filament-forms::rich-editor.toolbar.group
                                data-trix-button-group="history-tools"
                            >
                                @if ($hasToolbarButton('contentPicker'))
                                    <x-filament-forms::rich-editor.toolbar.button
                                        data-trix-action="x-content-picker"
                                        id="content-picker-button"
                                        title="Content Picker"
                                        tabindex="-1"
                                    >
                                        <x-filament::icon
                                            alias="inspirecms::content_picker"
                                            class="h-5 w-5"
                                        />
                                    </x-filament-forms::rich-editor.toolbar.button>
                                @endif
                                ...
                                @if ($hasToolbarButton('mediaPicker'))
                                    <x-filament-forms::rich-editor.toolbar.button
                                        data-trix-action="x-media-picker"
                                        id="media-picker-button"
                                        title="Media Picker"
                                        tabindex="-1"
                                    >
                                        <x-filament::icon
                                            alias="inspirecms::media_picker"
                                            class="h-5 w-5"
                                        />
                                    </x-filament-forms::rich-editor.toolbar.button>
                                @endif
