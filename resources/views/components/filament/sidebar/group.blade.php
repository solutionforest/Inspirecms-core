@php
    ...
    use SolutionForest\InspireCms\Filament\Navigation\NavigationItem;

    $ensureNavSectionKey = function ($navKey, $item) {
        if ($item instanceof NavigationItem) {
            return $item->getSection();
        }
        return isset($navKey) && filled($navKey) ? strtolower(trim($navKey)) : null;
    };

    $ensureNavItemKey = function ($item) {
        if ($item instanceof NavigationItem) {
    ...
    >
        @foreach ($items as $navKey => $item)
            ...
            <x-inspirecms::filament.sidebar.item
                ...
                :nav-item-key="$ensureNavItemKey($item)"
            >
                {{ $item->getLabel() }}
