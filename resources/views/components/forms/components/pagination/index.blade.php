...
@php
    ...
    $livewireKeyPrefix = $this->getId() . '_forms_' . $statePath . '_';

    $generateLivewireClickHandler = function ($action, $parameters = []) use ($statePath, $paginator) {
        
        $arguments = \Illuminate\Support\Js::from($parameters);

        return "mountFormComponentAction('{$statePath}', '{$action}', " . $arguments . ")";
    };
...
>
    @if (! $paginator->onFirstPage())
        <x-filament::button
            color="gray"
            rel="prev"
            :wire:click="$generateLivewireClickHandler('gotoPage', ['page' => $paginator->currentPage() - 1])"
            :wire:key="$livewireKeyPrefix . '.pagination.previous'"
        ...
        >
            @if (! $paginator->onFirstPage())
                @if ($extremeLinks)
                    <x-filament::pagination.item
                        ...
                        rel="first"
                        :wire:click="$generateLivewireClickHandler('gotoPage', ['page' => 1])"
                        :wire:key="$livewireKeyPrefix . '.pagination.first'"
                    />
                @endif
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        <x-filament::pagination.item
                            ...
                            :label="$page"
                            :wire:click="$generateLivewireClickHandler('gotoPage', ['page' => $page])"
                            :wire:key="$livewireKeyPrefix . '.pagination.' . $page"
                        />
                    @endforeach
                @endif
            @endforeach
            ...
            @if ($paginator->hasMorePages())
                <x-filament::pagination.item
                    ...
                    rel="next"
                    :wire:click="$generateLivewireClickHandler('gotoPage', ['page' => $paginator->currentPage() + 1])"
                    :wire:key="$livewireKeyPrefix . '.pagination.next'"
                />
                ...
                @if ($extremeLinks)
                    <x-filament::pagination.item
                        ...
                        rel="last"
                        :wire:click="$generateLivewireClickHandler('gotoPage', ['page' => $paginator->lastPage()])"
                        :wire:key="$livewireKeyPrefix . '.pagination.last'"
                    />
                @endif
            @endif
        </ol>
